...
export function EchoBg({ noteValues = {}, onNoteChange, highlightedField }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, fontFamily: "'Open Sans', Verdana, Arial, sans-serif", fontSize: 13, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* ── Orange top header ── */}
      <div style={{ height: 50, background: ORANGE, display: 'flex', alignItems: 'center', padding: '0 8px', flexShrink: 0, zIndex: 20 }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: 175, background: SIDEBAR_BG, display: 'flex', flexDirection: 'column', flexShrink: 0, zIndex: 10, overflowX: 'hidden', overflowY: 'auto' }}>
          {navItems.map(item => {
            const isHovered = hoveredNav === item.label;
            return (
              <div
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', background: '#fff', display: 'flex', flexDirection: 'column' }}>
            ...
            {/* Note fields */}
            <div style={{ padding: '20px 30px 80px', flex: 1 }}>
...
export function ExymBg({ noteValues = {}, onNoteChange, highlightedField }) {
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      {/* PNG chrome — fills entire container */}
      <img
        src="/exym-ehr-bg.png"
        alt=""
        style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'fill', display: 'block', pointerEvents: 'none' }}
      />
      {/* Clean white overlay for the No Show / Note Status section (PNG has a stale dropdown artifact here) */}
      <div style={{ position: 'absolute', top: '10%', left: '0.7%', right: '0.7%', height: '8.5%', background: '#fff', borderTop: '1px solid #e0e6ec', borderBottom: '1px solid #e0e6ec', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 16px', fontFamily: "'Segoe UI', Arial, sans-serif", fontSize: 13 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#333' }}>
          <span style={{ fontWeight: 600 }}>No Show:</span>
          <input type="checkbox" style={{ margin: 0 }} />
          <span style={{ marginLeft: 6, color: '#888' }}>Reason: — Client Absent</span>
        </div>
      ...
      {/* Functional note fields — overlaid on the note content area (~25.9% down) */}
      <div style={{ position: 'absolute', top: '25.9%', left: '0.7%', right: '0.7%', bottom: 0, background: '#fff', overflowY: 'auto', padding: '14px 18px' }}>
        <StackedFields noteValues={noteValues} onNoteChange={onNoteChange} highlightedField={highlightedField} labelColor='#444' fontSize={13} borderColor='#ccc' minHeight={130} borderRadius={3} />
      </div>
    </div>
  );
}
