...
function CompanionLaunchButton({ pos, onPosChange, onNext, onOpenQuality, isRecording }) {
  ...
  const circleFill = smartScribeSkin && isHovering ? 'rgba(37,74,103,0.6)' : smartScribeColor(smartScribeSkin);
    ...
    return (
      ...
      >
        ...
        <div style={{
          ...
          gap: 0, width: 68, height: 140, backgroundColor: smartScribeColor(smartScribeSkin), borderRadius: 34,
          ...
          <div style={{ position: 'relative', width: 52, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            {lqaStatus !== 'loading' && lqaStatus !== 'idle' && !(lqaStatus === 'success' && changedSinceAnalysis) && (
              <div style={{
                ...
                border: `2px solid ${smartScribeColor(smartScribeSkin)}`, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 4,
              }}>
                {lqaStatus === 'issues' && (
        ...
        <div style={{ position: 'absolute', top: '16.67%', right: '33.33%', bottom: '16.67%', left: '33.33%' }}>
          <div style={{ position: 'absolute', top: '-6.25%', right: '-12.5%', bottom: '-6.25%', left: '-12.5%' }}>
            <svg preserveAspectRatio="none" width="100%" height="100%" overflow="visible" viewBox="0 0 10 18" fill="none">
              <path d="M2 10C2.55228 10 3 9.55228 3 9C3 8.44772 2.55228 8 2 8C1.44772 8 1 8.44772 1 9C1 9.55228 1.44772 10 2 10Z" stroke={smartScribeColor(smartScribeSkin)} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M2 3C2.55228 3 3 2.55228 3 2C3 1.44772 2.55228 1 2 1C1.44772 1 1 1.44772 1 2C1 2.55228 1.44772 3 2 3Z" stroke={smartScribeColor(smartScribeSkin)} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M2 17C2.55228 17 3 16.5523 3 16C3 15.4477 2.55228 15 2 15C1.44772 15 1 15.4477 1 16C1 16.5523 1.44772 17 2 17Z" stroke={smartScribeColor(smartScribeSkin)} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M8 10C8.55228 10 9 9.55228 9 9C9 8.44772 8.55228 8 8 8C7.44772 8 7 8.44772 7 9C7 9.55228 7.44772 10 8 10Z" stroke={smartScribeColor(smartScribeSkin)} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M8 3C8.55228 3 9 2.55228 9 2C9 1.44772 8.55228 1 8 1C7.44772 1 7 1.44772 7 2C7 2.55228 7.44772 3 8 3Z" stroke={smartScribeColor(smartScribeSkin)} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M8 17C8.55228 17 9 16.5523 9 16C9 15.4477 8.55228 15 8 15C7.44772 15 7 15.4477 7 16C7 16.5523 7.44772 17 8 17Z" stroke={smartScribeColor(smartScribeSkin)} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        </div>
      </div>
...
function EHRBackground({ noteValues = INITIAL_NOTE_VALUES, onNoteChange, highlightedField, sidebarOpen, activitySelectionSeq }) {
  ...
  return <Bg noteValues={noteValues} onNoteChange={handleNoteChange} highlightedField={highlightedField} sidebarOpen={sidebarOpen} activitySelectionSeq={activitySelectionSeq} />;
}
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#EAEDFA', gap: 8, position: 'relative' }}>
        ...
        <div style={{ flex: 1, background: 'white', borderRadius: '16px 16px 0 0', boxShadow: SHADOW_EL16, overflowY: 'auto', padding: '0 24px 96px' }}>
          ...
          {/* Note Type */}
          <AcFormField key={summaryRule?.noteType ? 'locked-note' : 'note'} label="Note Type:" defaultValue="DAP Note" options={['DAP Note', 'SOAP Note', 'Progress Note', 'Treatment Plan', 'Intake Note', 'Medication Management', 'BPS Assessment']} compactMode={compactMode} forcedValue={summaryRule?.noteTypeLabel} disabled={!!summaryRule?.noteType} />
...
function NavRailIcon({ navKey, active, size = 24, skin = false }) {
  const c = active ? smartScribeColor(skin) : 'white';
  switch (navKey) {
    case 'activities':
      return (
...
function EleosNavRail({ activeItem, onNavClick, side, visibleItems, hasOverflow, overflowActive, onMoreClick, showMore, onResizeMouseDown, showLabels, isCapturing, onCollapse, compactMode = false }) {
  ...
  const ehrCtx = useEhrField();
  const smartScribeSkin = useSmartScribeSkin();
  const railNavy = smartScribeColor(smartScribeSkin);
