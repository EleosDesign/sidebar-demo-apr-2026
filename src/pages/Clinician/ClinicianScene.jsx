...
function EleosSidebar({ step, onNext, onCollapse, initialPos, savedState, onSaveState, onRecordingChange, onAddToNote, startTab, onStartTabConsumed }) {
  ...
  useEffect(() => {
    ...
    if (step === 1 && prevStep !== null && prevStep !== step) {
      ...
      setCapturePhase(null); setCaptureSession({ name: '', dateTime: 'Apr 2, 2026, 10:30 AM' });
    }
  }, [step]);
...
/// Add-on CPT badge: green
const BadgeAddOn = ({ compactMode = false }) => (
  <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 9999, padding: '3px 10px', display: 'flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap' }}>
    <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
      <rect x="2" y="2" width="12" height="12" rx="2" stroke="#166534" strokeWidth="1.5"/>
      <path d="M5.5 8h5M8 5.5v5" stroke="#166534" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
    <span style={{ fontSize: 11.5, fontWeight: 500, color: '#166534', fontFamily: 'Poppins, sans-serif' }}>{compactMode ? 'Add-On' : 'Add-On CPT Code'}</span>
  </div>
);
...
const PSYCH_COPY = {
  16: "16–37 minutes of individual psychotherapy provided in conjunction with the E/M service. Session addressed the patient's presenting behavioral health concerns.",
  30: "30–52 minutes of individual psychotherapy provided in conjunction with the E/M service. Extended session allowed for deeper exploration of the patient's therapeutic goals.",
  45: "45–59 minutes of individual psychotherapy provided in conjunction with the E/M service. Intensive session focused on complex behavioral health intervention.",
  60: "60+ minutes of individual psychotherapy provided in conjunction with the E/M service. Extended therapeutic engagement addressing significant psychiatric complexity.",
};

const TIER_OPTS = [16, 30, 45, 60];

function AddOnCptCard({ compactMode = false }) {
  const [tier, setTier]         = useState(30);
  const [tmpTier, setTmpTier]   = useState(30);
  const [isEditing, setIsEditing] = useState(false);
  const [psychOpen, setPsychOpen] = useState(true);

  const startEdit = () => { setTmpTier(tier); setIsEditing(true); };
  const cancel    = () => setIsEditing(false);
  const confirm   = () => { setTier(tmpTier); setIsEditing(false); };

  const activeTier = isEditing ? tmpTier : tier;

  const psychSection = (
    <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: 12, marginBottom: 4 }}>
      <div
        onClick={() => setPsychOpen(o => !o)}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: psychOpen ? 9 : 0, cursor: 'pointer', userSelect: 'none' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: '#4f5bd5', fontFamily: 'Poppins, sans-serif' }}>
          <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
            <rect x="1.5" y="3.5" width="13" height="9" rx="1.5" stroke="#4F5BD5" strokeWidth="1.4"/>
            <path d="M5 3.5V2M11 3.5V2" stroke="#4F5BD5" strokeWidth="1.4" strokeLinecap="round"/>
          </svg>
          Psychotherapy
        </div>
        <svg
          style={{ transform: psychOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s ease', flexShrink: 0 }}
      ...
      {psychOpen && (
        <div style={{ background: '#eef0ff', borderRadius: 8, padding: '10px 12px', fontSize: 13, lineHeight: 1.55, color: '#3730a3', fontFamily: 'Poppins, sans-serif' }}>
          {PSYCH_COPY[activeTier]}
        </div>
      )}
    </div>
  );

  const columns = [['Start', '10:00 A.M.'], ['End', '10:30 A.M.'], ['Duration', `${tier} min`]];
      ...
      {isEditing ? (
        <>
          ...
          <div style={{ marginBottom: 4 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: '#4f5bd5', fontFamily: 'Poppins, sans-serif', marginBottom: 10 }}>
              <IconClock />
              Time
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {TIER_OPTS.map(t => (
                <button key={t} onClick={() => setTmpTier(t)} style={{ ...cptChipBase, ...(tmpTier === t ? cptChipActive : {}) }}>
                  ≥{t} min
                </button>
              ))}
            </div>
          ...
          {psychSection}
          {/* Footer */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 14, paddingTop: 10, borderTop: '1px solid #f3f4f6' }}>
            <button onClick={cancel} style={{ background: 'none', border: 'none', color: 'rgba(0,0,0,0.45)', fontSize: 14, fontWeight: 500, fontFamily: 'Poppins, sans-serif', cursor: 'pointer' }}>Cancel</button>
            <div style={{ width: 1, height: 16, background: '#e5e7eb' }} />
            <button onClick={confirm} style={{ background: 'none', border: 'none', color: '#2d4ccd', fontSize: 14, fontWeight: 600, fontFamily: 'Poppins, sans-serif', cursor: 'pointer' }}>Confirm</button>
          </div>
        </>
      ) : (
        <>
          {/* Time row */}
          <div style={{ background: '#f8f9ff', border: '1px solid #e8eaff', borderRadius: 8, padding: '8px 12px', display: 'flex', flexDirection: compactMode ? 'column' : 'row', alignItems: 'stretch', gap: compactMode ? 0 : undefined }}>
            {columns.map(([label, val], i) => (
              <React.Fragment key={label}>
                {i > 0 && (
                  compactMode
                    ? <div style={{ height: 1, background: '#e5e7eb', margin: '6px 0' }} />
                    : <div style={{ width: 1, background: '#e5e7eb', margin: '0 8px', flexShrink: 0 }} />
                )}
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'rgba(0,0,0,0.5)', fontFamily: 'Poppins, sans-serif', fontWeight: 500, marginBottom: 2 }}>
                    <IconClock />{label}
                  </div>
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: 'rgba(0,0,0,0.87)', fontFamily: 'Poppins, sans-serif' }}>{val}</p>
                </div>
              </React.Fragment>
            ))}
          </div>
          ...
          <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: 12, display: 'flex', justifyContent: 'flex-end' }}>
            <button style={cptEditBtn} onClick={startEdit}><IconPencil />Edit Time</button>
          </div>
        </>
      )}
    </div>
  ...
  return (
    <div style={cptCard}>
      ...
      {confirmed ? (
        <>
          <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: 12, marginBottom: 4 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: '#4f5bd5', fontFamily: 'Poppins, sans-serif', marginBottom: 9 }}>
              <IconLayers />
        ...
        <>
          <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: 12, marginBottom: 4 }}>
            ...
            <textarea
              ...
              rows={6}
              style={{ width: '100%', boxSizing: 'border-box', resize: 'vertical', border: '1px solid #e8eaff', borderRadius: 8, padding: '10px 12px', fontFamily: 'Poppins, sans-serif', fontSize: 13, fontWeight: 400, color: '#3730a3', lineHeight: 1.55, background: '#eef0ff', outline: 'none' }}
            />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 14, paddingTop: 10, borderTop: '1px solid #f3f4f6' }}>
            <button style={{ background: 'none', border: 'none', color: 'rgba(0,0,0,0.45)', fontSize: 14, fontWeight: 500, fontFamily: 'Poppins, sans-serif', cursor: 'pointer' }} onClick={() => { setDraft(narrative); setConfirmed(true); }}>Cancel</button>
            <div style={{ width: 1, height: 16, background: '#e5e7eb' }} />
            <button style={{ background: 'none', border: 'none', color: '#2d4ccd', fontSize: 14, fontWeight: 600, fontFamily: 'Poppins, sans-serif', cursor: 'pointer' }} onClick={() => { setNarrative(draft); setConfirmed(true); }}>Confirm</button>
          </div>
        </>
      )}
    </div>
